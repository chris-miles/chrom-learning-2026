...
- `00b_supplement.py` -- supplement figure assembler. Renders four standalone figures into `figures/supplement/` (PDF + 600 dpi PNG), each load-bearing for a specific main-text claim: **S1** (per-cell `f_pp` and `f_cp` from the pp+cp pole-velocity regression with chromosomes as observed covariates, plus the constrained-share refit sweep on the spindle-separation observable showing the `(f_pp, f_cp)` partition is non-identifiable -- companion to Fig 2); **S2** (from-NEB ensemble MSE vs horizon 1-30 frames for all five topologies, h=10 marked as Alex's docx Result-3C anchor -- companion to Fig 3); **S3** (hyperparameter and convention sensitivity on the canonical `poles_and_chroms_enveloped` topology: `(n_basis × λ_rough)` path-MSE heatmap, Itô vs Stratonovich bars, endpoint-method bars with cell-attrition annotation -- companion to Fig 3); **S4** (per-cell `f_xy` and enveloped `f_xx` kernel spaghetti over pooled bootstrap 5-95 % CI for the selected topology -- companion to Fig 3). Aesthetics mirror `00_main_figure.py` (Okabe-Ito palette, multi-line topology labels, kernel y-labels with `(μm/s)` and `+ attractive · - repulsive` sign convention, separate bold panel letters).
