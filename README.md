...
- `notebooks/` — Jupytext percent-format `.py` notebooks (source of truth for all edits)
  ...
  - `04_model_selection.py` — Compares 5 interaction topologies via leave-one-cell-out CV (primary: ensemble-mean MSE with paired foldwise Δ/SE(Δ); supporting: per-rep path MSE, 1-step velocity MSE, W1, horizon-resolved error curves), metric concordance analysis, kernel plausibility, and forward simulation
...
- **XX cutoff** (`r_cutoff_xx`): If set (e.g. 2.5 um), chromosome-chromosome forces are zeroed beyond this distance in both fitting and simulation. Tests whether xx benefit is local steric repulsion vs long-range nuisance absorption
