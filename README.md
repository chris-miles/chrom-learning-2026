...
- `notebooks/` — Jupytext percent-format `.py` notebooks (source of truth for all edits)
  ...
  - **Paper-figure assemblers** (primary interface for paper output):
    ...
    - `00b_supplement.py` — Supplement figure assembler. Four panels, each tied to a specific main-text claim: S1 (per-cell pp/cp pole-velocity kernels + constrained-share refit sweep showing partition non-identifiability — companion to Fig 2), S2 (from-NEB ensemble MSE vs horizon 1-30 frames across all 5 topologies, h=10 docx anchor — companion to Fig 3), S3 (hyperparameter heatmap + Itô/Stratonovich bars + endpoint sweep with cell-attrition annotation — companion to Fig 3), S4 (per-cell envelope kernels over pooled bootstrap CI — companion to Fig 3).
