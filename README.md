...
- `notebooks/` — Jupytext percent-format `.py` notebooks (source of truth for all edits)
  ...
  - **Paper-figure assemblers** (primary interface for paper output):
    - `00_main_figure.py` — Main-text figure assembler. Renders four standalone figures into `figures/main/` (PDF + 600 dpi PNG): Fig 1 (PCA + lag), Fig 2 (CS-CS sufficient with cp-only ablation: f_pp + 46·f_cp + cp-only overlay panels, three-bar path-MSE [pp-only / pp+cp / cp-only]), Fig 3 (force kernels + sorted topology path-MSE bars), Fig 3b (in-sample sim-vs-real PCA validation), Fig 4 (two-panel: D(d) + drift signal fraction f_drift(d; T_obs)). Per-cell strip plots punted to supplement.
    - `00b_supplement.py` — Supplement figure assembler. Five panels, each tied to a specific main-text claim: S1 (per-cell pp/cp pole-velocity kernels + constrained-share refit sweep showing partition non-identifiability — companion to Fig 2), S2 (from-NEB ensemble MSE vs horizon 1-30 frames across all 5 topologies, h=10 docx anchor — companion to Fig 3), S3 (2×2 kernel-vs-hyperparameter sweep + Itô/Stratonovich kernel comparison — companion to Fig 3), S4 (per-cell envelope kernels over pooled bootstrap CI — companion to Fig 3), S5 (drift-vs-diffusion sensitivity: T-sweep f_drift(d; T) + per-step Pe + tau_50(d) — companion to Fig 4).
