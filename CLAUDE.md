...
- `00b_supplement.py` -- supplement figure assembler. Skeleton; panel implementations land here as the main text stabilises. Planned panels are organised by which main-text figure they support: **S1** (per-cell pp-only vs pp+cp strip plot for both 1-step RMSE and path MSE -- companion to Fig 2); **S2** (NB03b pp/cp partition non-identifiability reconciliation -- companion to Fig 2); **S3** (per-cell 5-topology path-MSE strip plot including `center_and_chroms` -- companion to Fig 3); **S4** (forecast error vs horizon 1-30 frames for all 5 topologies -- companion to Fig 3 and Alex's docx anchor at h=10); **S5** (NB05 hyperparameter heatmap + Itô/Itô-shift/Strato bars + endpoint bars -- companion to Fig 3); **S6** (NB07 per-cell kernel variability spaghetti -- companion to Fig 3); **S7** (D-estimator robustness: f_corrected / Vestergaard / MSD / weak_noise overlay -- companion to Fig 4).
