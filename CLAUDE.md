...
- **Short-range xx test**: The `poles_and_chroms_short` topology uses `r_cutoff_xx` (default 2.5 um) to restrict chromosome-chromosome forces to short range, testing whether the xx benefit is local steric repulsion vs a long-range nuisance absorber. The cutoff is enforced via explicit distance masks in both feature construction and simulation (not via basis `r_max`, since basis evaluation clamps rather than zeros out-of-domain distances).
